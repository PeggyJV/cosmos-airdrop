...
use futures::Future;
use ocular::{account::AccountInfo, cosmrs::{crypto::secp256k1::SigningKey, dev, rpc::HttpClient}};

pub type BroadcastCommitResponse = ocular::cosmrs::rpc::endpoint::broadcast::tx_commit::Response;

/// Chain ID to use for tests
pub const CHAIN_ID: &str = "cosmos-airdrop-test-chain";
    ...
    Fut: Future<Output = ()>,
{
    ...
    let sender_account = AccountInfo::from(key);
    ...
    // wait for chain to come up
    dev::poll_for_first_block(&HttpClient::new(format!("http://localhost:{}", RPC_PORT).as_str()).unwrap()).await;
